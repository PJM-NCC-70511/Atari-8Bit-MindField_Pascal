...
procedure Main_Game_Loop;
    begin
    repeat 				
            if prior_rt_clock <> RT_CHECK then 
                begin				              
								...
								if character_status = 0 then begin 
									...
									f:=0;  
						          	...
						          	if (stick_read and 2=0) then begin
						          		character_py := character_py + 1;
						          		if (character_py>232) then character_py := 232;
						          		f:=3+G;
									end;
						          	if (stick_read and 4=0) then begin
										character_px := character_px - 1;
										if (character_px<48) then character_px := 48;
										f:=2+G;
						          	end;
						          	if (stick_read and 8=0) then begin
						          		character_px := character_px + 1;
						          		if (character_px>200) then character_px := 200;
						          		f:=4+G;
						          	end;															            						            			
									a := (character_px - 47) DIV 4;
									b := (character_py - 24) DIV 8;
...
// Game Initialization here
	    ...
	    repeat		
			...
			f :=0; 				
 		    prior_rt_clock := RT_CHECK;
	  		...
	  		if (character_status = 128) then begin 
              lives := lives-1; 
              if (lives = 255) then begin;
	    ...
	    until (character_status = 159);
	    if (character_status = 159) then begin
			repeat 				
				if prior_rt_clock <> RT_CHECK then begin
					character_status := character_status - 1;
					prior_rt_clock := RT_CHECK;
				end;
			until (character_status = 80);
		end; 						              
	until false
